...
export class Scanner extends Component {
  ...
  startCamera() {
    if (!this.camera) {
      ...
      this.camera.applySettings(this.props.cameraSettings);
    }
    ...
    this.idCaptureListener = {
      didCaptureId: (_, session) => {
        ...
        if (session.newlyCapturedId.mrzResult != null) {
          ...
          result = this.descriptionForUsDriverLicenseBarcodeResult(
            session.newlyCapturedId
          );
        } else if (
          session.newlyCapturedId.usUniformedServicesBarcodeResult != null
        ) {
          result = this.descriptionForUsUniformedServicesBarcodeResult(
            session.newlyCapturedId
          );
        } else if (
          session.newlyCapturedId.apecBusinessTravelCardMrzResult != null
        ) {
          result = this.descriptionForApecBusinessTravelCardMrzResult(
            session.newlyCapturedId
          );
        } else if (
          session.newlyCapturedId.chinaOneWayPermitFrontMrzResult != null
        ) {
          result = this.descriptionForChinaOneWayPermitFrontMrzResult(
            session.newlyCapturedId
          );
        } else if (
          session.newlyCapturedId.chinaOneWayPermitBackMrzResult != null
        ) {
          result = this.descriptionForChinaOneWayPermitBackMrzResult(
            session.newlyCapturedId
          );
        } else {
  ...
  descriptionForVizResult(result) {
    ...
    Additional Name Information: ${
      result.vizResult.additionalNameInformation || "empty"
    }
    Additional Address Information: ${
      result.vizResult.additionalAddressInformation || "empty"
    }
    ...
    Document Additional Number: ${
      result.vizResult.documentAdditionalNumber || "empty"
    }
  ...
  descriptionForUsDriverLicenseBarcodeResult(result) {
    ...
    Issuing Jurisdiction ISO: ${
      result.aamvaBarcodeResult.issuingJurisdictionISO
    }
    ...
    Document Discriminator Number: ${
      result.aamvaBarcodeResult.documentDiscriminatorNumber || "empty"
    }
    ...
    Card Revision Date: ${this.getDateAsString(
      result.aamvaBarcodeResult.cardRevisionDate
    )}
    ...
    Last Name Truncation: ${
      result.aamvaBarcodeResult.lastNameTruncation || "empty"
    }
    First Name Truncation: ${
      result.aamvaBarcodeResult.firstNameTruncation || "empty"
    }
    Middle Name Truncation: ${
      result.aamvaBarcodeResult.middleNameTruncation || "empty"
    }
  ...
  descriptionForUsUniformedServicesBarcodeResult(result) {
    ...
    Person Designator Document: ${
      result.usUniformedServicesBarcodeResult.personDesignatorDocument
    }
    Family Sequence Number: ${
      result.usUniformedServicesBarcodeResult.familySequenceNumber
    }
    Deers Dependent Suffix Code: ${
      result.usUniformedServicesBarcodeResult.deersDependentSuffixCode
    }
    Deers Dependent Suffix Description: ${
      result.usUniformedServicesBarcodeResult.deersDependentSuffixDescription
    }
    ...
    Direct Care Flag Code: ${
      result.usUniformedServicesBarcodeResult.directCareFlagCode
    }
    Direct Care Flag Description: ${
      result.usUniformedServicesBarcodeResult.directCareFlagDescription
    }
    Civilian Health Care Flag Code: ${
      result.usUniformedServicesBarcodeResult.civilianHealthCareFlagCode
    }
    Civilian Health Care Flag Description: ${
      result.usUniformedServicesBarcodeResult.civilianHealthCareFlagDescription
    }
    Commissary Flag Code: ${
      result.usUniformedServicesBarcodeResult.commissaryFlagCode
    }
    Commissary Flag Description: ${
      result.usUniformedServicesBarcodeResult.commissaryFlagDescription
    }
    MWR Flag Code: ${result.usUniformedServicesBarcodeResult.mwrFlagCode}
    MWR Flag Description: ${
      result.usUniformedServicesBarcodeResult.mwrFlagDescription
    }
    Exchange Flag Code: ${
      result.usUniformedServicesBarcodeResult.exchangeFlagCode
    }
    Exchange Flag Description: ${
      result.usUniformedServicesBarcodeResult.exchangeFlagDescription
    }
    Champus Effective Date: ${this.getDateAsString(
      result.usUniformedServicesBarcodeResult.champusEffectiveDate
    )}
    Champus Expiry Date: ${this.getDateAsString(
      result.usUniformedServicesBarcodeResult.champusExpiryDate
    )}
    ...
    Status Code Description: ${
      result.usUniformedServicesBarcodeResult.statusCodeDescription
    }
    Branch Of Service: ${
      result.usUniformedServicesBarcodeResult.branchOfService
    }
    Rank: ${result.usUniformedServicesBarcodeResult.rank}
    Pay Grade: ${result.usUniformedServicesBarcodeResult.payGrade}
    Sponsor Name: ${
      result.usUniformedServicesBarcodeResult.sponsorName || "empty"
    }
    Sponsor Person Designator Identifier: ${
      result.usUniformedServicesBarcodeResult
        .sponsorPersonDesignatorIdentifier || 0
    }
    Relationship Code: ${
      result.usUniformedServicesBarcodeResult.relationshipCode || "empty"
    }
    Relationship Description: ${
      result.usUniformedServicesBarcodeResult.relationshipDescription || "empty"
    }
    Geneva Convention Category: ${
      result.usUniformedServicesBarcodeResult.genevaConventionCategory ||
      "empty"
    }
    Blood Type: ${
      result.usUniformedServicesBarcodeResult.bloodType || "empty"
    }`;
  }
  ...
  descriptionForChinaOneWayPermitFrontMrzResult(result) {
    ...
    Full Name Simplified Chinese: ${
      result.chinaOneWayPermitFrontMrzResult.fullNameSimplifiedChinese
    }
  ...
  descriptionForChinaOneWayPermitBackMrzResult(result) {
    ...
    Names Are Truncated: ${
      result.chinaOneWayPermitBackMrzResult.namesAreTruncated ? "Yes" : "No"
    }
  ...
  descriptionForApecBusinessTravelCardMrzResult(result) {
    ...
    Names Are Truncated: ${
      result.apecBusinessTravelCardMrzResult.namesAreTruncated ? "Yes" : "No"
    }
    Passport Issuer Iso: ${
      result.apecBusinessTravelCardMrzResult.passportIssuerIso
    }
    Passport Number: ${result.apecBusinessTravelCardMrzResult.passportNumber}
    Passport Date Of Expiry: ${this.getDateAsString(
      result.apecBusinessTravelCardMrzResult.passportDateOfExpiry
    )}`;
  }
  ...
  getDateAsString(dateObject) {
    return dateObject && dateObject.localDate
      ? dateObject.localDate.toLocaleDateString("en-GB")
      : "empty";
  }
